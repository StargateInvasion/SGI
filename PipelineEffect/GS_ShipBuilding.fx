...
float4 GetFinalPixelColorPBR(float2 texCoord, float3 pos, float3 normal, float percentHeight)
{   
...
//	float3 DiffuseDirection 	= GetDiffuseDominantDir(normal, view, NoV, Properties);
	...
	{
		finalColor.rgb += Pow5(1.0 - NoV);
		finalColor.rgb *= float3(0.0, 1.0, 1.0);
	}
...
{ 
	return GetFinalPixelColorPBR(input.TexCoord, input.PosWS, input.NormalWS, input.PercentHeight);
}
